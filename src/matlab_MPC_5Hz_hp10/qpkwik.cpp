...
namespace pos_MPC {
namespace coder {
static void DropConstraint(short kDrop, short iA[120], short *nA,
                           short iC[120]);

static double KWIKfactor(const double Ac[3720], const short iC[120], short nA,
                         const double Linv[961], double RLinv[961],
                         double D[961], double H[961]);
...
namespace pos_MPC {
namespace coder {
static void DropConstraint(short kDrop, short iA[120], short *nA, short iC[120])
{
  iA[iC[kDrop - 1] - 1] = 0;
  if (kDrop < *nA) {
    short i;
                         ...
                         double D[961], double H[961])
{
  ...
  boolean_T exitg2;
  Status = 1.0;
  std::memset(&RLinv[0], 0, 961U * sizeof(double));
  i = nA;
  for (b_i = 0; b_i < i; b_i++) {
    for (i1 = 0; i1 < 31; i1++) {
      i2 = i1 + 31 * b_i;
      RLinv[i2] = 0.0;
      for (ix0 = 0; ix0 < 31; ix0++) {
        RLinv[i2] += Linv[i1 + 31 * ix0] * Ac[(iC[b_i] + 120 * ix0) - 1];
      }
    }
  }
  std::copy(&RLinv[0], &RLinv[961], &A[0]);
  std::memset(&tau[0], 0, 31U * sizeof(double));
  std::memset(&work[0], 0, 31U * sizeof(double));
  for (b_i = 0; b_i < 31; b_i++) {
    ...
    if (b_i + 1 < 31) {
      ...
      ix0 = ii + 2;
      tau[b_i] = 0.0;
      c = internal::blas::xnrm2(30 - b_i, A, ii + 2);
      if (c != 0.0) {
        double beta1;
        double d;
        d = A[ii];
        ...
        if (std::abs(beta1) < 1.0020841800044864E-292) {
          ...
          beta1 =
              rt_hypotd_snf(atmp, internal::blas::xnrm2(30 - b_i, A, ii + 2));
          if (atmp >= 0.0) {
            beta1 = -beta1;
          }
      ...
      if (lastv > 0) {
        ...
        if (lastc != 0) {
          ...
          i1 = (ii + 31 * (lastc - 1)) + 32;
          for (iac = ix0; iac <= i1; iac += 31) {
            c = 0.0;
            i2 = (iac + lastv) - 1;
            for (ia = iac; ia <= i2; ia++) {
              c += A[ia - 1] * A[(ii + ia) - iac];
            }
            knt = div_nde_s32_floor((iac - ii) - 32, 31);
            work[knt] += c;
          }
        }
  ...
  for (ix0 = 0; ix0 < 31; ix0++) {
    ...
    i1 = ix0 + 2;
    if (i1 <= 31) {
      std::memset(&R[(ix0 * 31 + i1) + -1], 0, (32 - i1) * sizeof(double));
    }
    work[ix0] = 0.0;
  }
    ...
    if (b_i + 1 < 31) {
      ...
      if (lastv > 0) {
        if (lastc != 0) {
          ...
          i1 = ii + 31 * (lastc - 1);
          for (iac = ii; iac <= i1; iac += 31) {
            c = 0.0;
            i2 = (iac + lastv) - 1;
            for (ia = iac; ia <= i2; ia++) {
              c += A[ia - 1] * A[((ii + ia) - iac) - 32];
            }
            knt = div_nde_s32_floor(iac - ii, 31);
            work[knt] += c;
          }
        }
        internal::blas::xgerc(lastv, lastc, -tau[b_i], ii - 31, work, A, ii);
      }
      ix0 = ii - 30;
      i1 = (ii - b_i) - 1;
      for (lastc = ix0; lastc <= i1; lastc++) {
  ...
  for (ix0 = 0; ix0 < 31; ix0++) {
    std::copy(&A[ix0 * 31], &A[static_cast<int>(ix0 * 31 + 31U)], &Q[ix0 * 31]);
  }
  b_i = 0;
  do {
    exitg1 = 0;
    if (b_i <= nA - 1) {
      if (std::abs(R[b_i + 31 * b_i]) < 1.0E-12) {
        Status = -2.0;
        exitg1 = 1;
      } else {
        b_i++;
            ...
            double x[31], double lambda[120], double *status)
{
  ...
  *status = 1.0;
  std::memset(&lambda[0], 0, 120U * sizeof(double));
  std::memset(&x[0], 0, 31U * sizeof(double));
  std::memset(&r[0], 0, 31U * sizeof(double));
  rMin = 0.0;
  cTolComputed = false;
  for (i = 0; i < 120; i++) {
    cTol[i] = 1.0;
  ...
  if (nA > 0) {
    ...
    unsigned short b_x;
    unsigned short q;
    std::memset(&Opt[0], 0, 62U * sizeof(double));
    for (i = 0; i < 31; i++) {
      Rhs[i] = f[i];
      Rhs[i + 31] = 0.0;
    }
    ...
    do {
      exitg3 = 0;
      if ((!DualFeasible) && (nA > 0) && (*status <= 604.0)) {
        ...
        if (cVal < 0.0) {
          if (ColdReset) {
            *status = -2.0;
            ...
            nA = 0;
            std::memset(&iA[0], 0, 120U * sizeof(short));
            std::memset(&iC[0], 0, 120U * sizeof(short));
            ColdReset = true;
          }
        } else {
          b_i = nA;
          for (int j{0}; j < b_i; j++) {
            ...
            Rhs[static_cast<short>(j + 32) - 1] = b[iC[j] - 1];
          ...
          for (i = 0; i < 31; i++) {
            cVal = 0.0;
            for (i1 = 0; i1 < 31; i1++) {
              cVal += H[i + 31 * i1] * Rhs[i1];
            }
            Opt[i] = cVal;
            for (k = 0; k < b_i; k++) {
              Opt[i] += D[i + 31 * k] * Rhs[k + 31];
            }
          }
          b_i = nA;
          for (i = 0; i < b_i; i++) {
            cVal = 0.0;
            for (i1 = 0; i1 < 31; i1++) {
              cVal += D[i1 + 31 * i] * Rhs[i1];
            }
            Opt[i + 31] = cVal;
            i1 = nA;
            for (k = 0; k < i1; k++) {
              Opt[i + 31] += U[i + 31 * k] * Rhs[k + 31];
            }
          }
          cVal = -1.0E-12;
          kDrop = 0;
          b_i = nA;
          for (i = 0; i < b_i; i++) {
            cMin = Opt[static_cast<short>(i + 32) - 1];
            lambda[iC[i] - 1] = cMin;
            if ((cMin < cVal) && (i + 1 <= nA)) {
          ...
          if ((kNext > 0) && (*status <= 604.0)) {
            ...
            if (nA == 0) {
              for (b_i = 0; b_i < 31; b_i++) {
                cMin = 0.0;
                for (i1 = 0; i1 < 31; i1++) {
                  cMin += Hinv[b_i + 31 * i1] * Ac[(kNext + 120 * i1) - 1];
                }
                z[b_i] = cMin;
              }
              guard2 = true;
              ...
              if (cVal <= 0.0) {
                *status = -2.0;
                ...
                for (b_i = 0; b_i < 961; b_i++) {
                  U[b_i] = -H[b_i];
                }
                for (b_i = 0; b_i < 31; b_i++) {
                  cMin = 0.0;
                  for (i1 = 0; i1 < 31; i1++) {
                    cMin += U[b_i + 31 * i1] * Ac[(kNext + 120 * i1) - 1];
                  }
                  z[b_i] = cMin;
                }
                b_i = nA;
                for (i = 0; i < b_i; i++) {
                  b_Ac = 0.0;
                  for (i1 = 0; i1 < 31; i1++) {
                    b_Ac += Ac[(kNext + 120 * i1) - 1] * D[i1 + 31 * i];
                  }
                  r[i] = b_Ac;
                }
                guard2 = true;
