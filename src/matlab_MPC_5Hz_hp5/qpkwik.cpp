...
static double KWIKfactor(const double Ac[960], const short iC[60], short nA,
                         const double Linv[256], double RLinv[256],
                         double D[256], double H[256]);
                         ...
                         double D[256], double H[256])
{
  ...
  boolean_T exitg2;
  Status = 1.0;
  std::memset(&RLinv[0], 0, 256U * sizeof(double));
  i = nA;
  for (b_i = 0; b_i < i; b_i++) {
    for (i1 = 0; i1 < 16; i1++) {
      i2 = i1 + (b_i << 4);
      RLinv[i2] = 0.0;
      for (ix0 = 0; ix0 < 16; ix0++) {
        RLinv[i2] += Linv[i1 + (ix0 << 4)] * Ac[(iC[b_i] + 60 * ix0) - 1];
      }
    }
  }
  std::copy(&RLinv[0], &RLinv[256], &A[0]);
  std::memset(&tau[0], 0, 16U * sizeof(double));
  ...
  for (b_i = 0; b_i < 16; b_i++) {
    ...
    if (b_i + 1 < 16) {
      ...
      ix0 = ii + 2;
      tau[b_i] = 0.0;
      c = internal::blas::xnrm2(15 - b_i, A, ii + 2);
      if (c != 0.0) {
        double beta1;
        double d;
        d = A[ii];
        ...
        if (std::abs(beta1) < 1.0020841800044864E-292) {
          ...
          beta1 =
              rt_hypotd_snf(atmp, internal::blas::xnrm2(15 - b_i, A, ii + 2));
          if (atmp >= 0.0) {
            beta1 = -beta1;
          }
  ...
  for (ix0 = 0; ix0 < 16; ix0++) {
    ...
    i1 = ix0 + 2;
    if (i1 <= 16) {
      std::memset(&R[(ix0 * 16 + i1) + -1], 0, (17 - i1) * sizeof(double));
    }
    work[ix0] = 0.0;
  }
  for (b_i = 15; b_i >= 0; b_i--) {
    ii = (b_i + (b_i << 4)) + 17;
    if (b_i + 1 < 16) {
      A[ii - 17] = 1.0;
      if (tau[b_i] != 0.0) {
      ...
      if (lastv > 0) {
        if (lastc != 0) {
          ...
          i1 = ii + ((lastc - 1) << 4);
          for (iac = ii; iac <= i1; iac += 16) {
            c = 0.0;
            i2 = (iac + lastv) - 1;
            for (ia = iac; ia <= i2; ia++) {
              c += A[ia - 1] * A[((ii + ia) - iac) - 17];
            }
            knt = (iac - ii) >> 4;
            work[knt] += c;
          }
        }
        internal::blas::xgerc(lastv, lastc, -tau[b_i], ii - 16, work, A, ii);
      }
      ix0 = ii - 15;
      i1 = (ii - b_i) - 1;
      for (lastc = ix0; lastc <= i1; lastc++) {
  ...
  for (ix0 = 0; ix0 < 16; ix0++) {
    std::copy(&A[ix0 * 16], &A[static_cast<int>(ix0 * 16 + 16U)], &Q[ix0 * 16]);
  }
  b_i = 0;
  do {
    exitg1 = 0;
    if (b_i <= nA - 1) {
      if (std::abs(R[b_i + (b_i << 4)]) < 1.0E-12) {
        Status = -2.0;
        exitg1 = 1;
      } else {
        b_i++;
            ...
            double x[16], double lambda[60], double *status)
{
  ...
  *status = 1.0;
  std::memset(&lambda[0], 0, 60U * sizeof(double));
  std::memset(&x[0], 0, 16U * sizeof(double));
  ...
  if (nA > 0) {
    ...
    unsigned short b_x;
    unsigned short q;
    std::memset(&Opt[0], 0, 32U * sizeof(double));
    for (i = 0; i < 16; i++) {
      Rhs[i] = f[i];
      Rhs[i + 16] = 0.0;
    }
    ...
    do {
      exitg3 = 0;
      if ((!DualFeasible) && (nA > 0) && (*status <= 304.0)) {
        ...
        if (cVal < 0.0) {
          if (ColdReset) {
            *status = -2.0;
            ...
            nA = 0;
            std::memset(&iA[0], 0, 60U * sizeof(short));
            std::memset(&iC[0], 0, 60U * sizeof(short));
            ColdReset = true;
          }
        } else {
          b_i = nA;
          for (int j{0}; j < b_i; j++) {
            ...
            Rhs[static_cast<short>(j + 17) - 1] = b[iC[j] - 1];
          ...
          for (i = 0; i < 16; i++) {
            cVal = 0.0;
            for (i1 = 0; i1 < 16; i1++) {
              cVal += H[i + (i1 << 4)] * Rhs[i1];
            }
            Opt[i] = cVal;
            for (k = 0; k < b_i; k++) {
              Opt[i] += D[i + (k << 4)] * Rhs[k + 16];
            }
          }
          b_i = nA;
          for (i = 0; i < b_i; i++) {
            cVal = 0.0;
            for (i1 = 0; i1 < 16; i1++) {
              cVal += D[i1 + (i << 4)] * Rhs[i1];
            }
            Opt[i + 16] = cVal;
            i1 = nA;
            for (k = 0; k < i1; k++) {
              Opt[i + 16] += U[i + (k << 4)] * Rhs[k + 16];
            }
          }
          cVal = -1.0E-12;
          kDrop = 0;
          b_i = nA;
          for (i = 0; i < b_i; i++) {
            cMin = Opt[static_cast<short>(i + 17) - 1];
            lambda[iC[i] - 1] = cMin;
            if ((cMin < cVal) && (i + 1 <= nA)) {
          ...
          if ((kNext > 0) && (*status <= 304.0)) {
            ...
            if (nA == 0) {
              for (b_i = 0; b_i < 16; b_i++) {
                cMin = 0.0;
                for (i1 = 0; i1 < 16; i1++) {
                  cMin += Hinv[b_i + (i1 << 4)] * Ac[(kNext + 60 * i1) - 1];
                }
                z[b_i] = cMin;
              }
              guard2 = true;
              ...
              if (cVal <= 0.0) {
                *status = -2.0;
                ...
                for (b_i = 0; b_i < 16; b_i++) {
                  cMin = 0.0;
                  for (i1 = 0; i1 < 16; i1++) {
                    cMin += U[b_i + (i1 << 4)] * Ac[(kNext + 60 * i1) - 1];
                  }
                  z[b_i] = cMin;
                }
                b_i = nA;
                for (i = 0; i < b_i; i++) {
                  b_Ac = 0.0;
                  for (i1 = 0; i1 < 16; i1++) {
                    b_Ac += Ac[(kNext + 60 * i1) - 1] * D[i1 + (i << 4)];
                  }
                  r[i] = b_Ac;
                }
                guard2 = true;
